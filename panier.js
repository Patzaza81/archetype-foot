...
function construitCartePanier(item,signal,index){
  if(signal&&aAuMoinsUnCandidat(signal))return construitCarte(signal,{replie:index>0,action:{libelle:"Retirer",aria:"Retirer du panier",onClick:()=>retirerDuPanier(item)}});
  return construitEtatVide(item)}
function affichePanier(){const root=document.getElementById("panier-cartes"),statut=document.getElementById("statut-panier"),panier=panierUnique(chargePanier());if(!panier.length){statut.textContent="Ton panier est vide.";root.innerHTML="";return}sauvePanier(panier);statut.textContent="Chargement…";fetch(`precalcul_leger.json?_=${Date.now()}`).then(r=>{if(!r.ok)throw new Error(`precalcul_leger.json introuvable (${r.status})`);return r.json()}).then(d=>{const signaux=regroupeMatchs(d.signaux||[]);const parId=new Map(signaux.map(s=>[identitePanier(s),s]));root.innerHTML="";let nbAvecSelection=0;panier.forEach((item,index)=>{const signal=parId.get(identitePanier(item));if(signal&&aAuMoinsUnCandidat(signal))nbAvecSelection++;root.appendChild(construitCartePanier(item,signal,index))});statut.textContent=`${panier.length} match${panier.length>1?"s":""} dans le panier — ${nbAvecSelection} avec une sélection Archetype.`}).catch(e=>{statut.textContent="Erreur de chargement : "+e.message;console.error(e)})}
affichePanier();
