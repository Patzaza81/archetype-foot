...
print("\n" + "=" * 70)
# ============================================================================
# CHANTIER "construit_etat_systeme.py + systeme.html" (13/09/2026, réécrit le
# 22/09/2026 -- NETTOYAGE demandé par Patrick : calibration adaptative et
# tickets fictifs supprimés entièrement, ce fichier ne consolide plus que
# les deux bilans comportementaux (moteur_v2_6_9 + shrink_v1, en test).
# Les vérités ci-dessous (paramètres calibrés, promotions, tickets) ont été
# retirées avec le code qu'elles testaient -- pas de repli, pas de test
# maintenu sur du code qui n'existe plus.
# ============================================================================
section("construit_etat_systeme.py (réécrit le 22/09/2026) -- consolide "
        "uniquement les deux bilans comportementaux, testé sur de vrais "
        "fichiers temporaires.")

import importlib as _importlib_ces
import json as _json_ces
import os as _os_ces
import tempfile as _tempfile_ces
import construit_etat_systeme as _ces_dyn
_importlib_ces.reload(_ces_dyn)   # au cas où une exécution précédente de ce script l'aurait déjà importé

try:
    with _tempfile_ces.TemporaryDirectory() as _d_ces:
        _cwd_avant_ces = _os_ces.getcwd()
        _os_ces.chdir(_d_ces)
        try:
            _etat_vide = _ces_dyn.construit_etat()
            verite(
                "construit_etat CAS aucun fichier de bilan présent (rejet attendu, cas honnête) : "
                "bilan_comportemental et bilan_shrink_v1 valent tous deux {} -- jamais une donnée "
                "inventée pour combler un vide, et plus aucun champ de calibration/tickets",
                _etat_vide["bilan_comportemental"] == {} and _etat_vide["bilan_shrink_v1"] == {}
                and "parametres_actifs" not in _etat_vide and "rapport_tickets_observation" not in _etat_vide,
            )

            with open("bilan_archetype_model.json", "w", encoding="utf-8") as _f_bilan_ces:
                _json_ces.dump({"global": {"observations": 42, "roi_flat": 0.05}, "par_famille": {}}, _f_bilan_ces)
            with open("bilan_shrink_v1.json", "w", encoding="utf-8") as _f_bilan_shrink_ces:
                _json_ces.dump({"global": {"observations": 7, "roi_flat": -0.05}, "par_famille": {}}, _f_bilan_shrink_ces)
            _etat_deux_bilans = _ces_dyn.construit_etat()
            verite(
                "construit_etat (doit réussir) : reprend tel quel le contenu de bilan_archetype_model.json "
                "ET bilan_shrink_v1.json, jamais recalculé ici, jamais l'un à la place de l'autre",
                _etat_deux_bilans["bilan_comportemental"]["global"]["observations"] == 42
                and _etat_deux_bilans["bilan_shrink_v1"]["global"]["observations"] == 7,
            )
        finally:
            _os_ces.chdir(_cwd_avant_ces)
