...
**Effet à connaître** : en début de saison les échantillons par lieu sont petits (médiane 4 matchs à domicile, 3 à l'extérieur) ; 51 des 74 choix du rejeu portent l'avertissement « Fenêtre d'analyse trop courte ». Aucun seuil minimal n'est appliqué (le moteur ne l'exige pas) : **décision à prendre** — l'ancien chargeur refusait sous 5 matchs (`validation.N_MIN_UTILISABLE`).
