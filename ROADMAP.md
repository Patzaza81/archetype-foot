...
| A3 | Correspondance des noms d'équipes football-data ↔ matchendirect ↔ BetPawa | aucune correspondance ambiguë acceptée ; les non-résolues listées | **FAIT le 24/09** (`assemblage_equipes.py`) : 529 équipes reliées dans 34 divisions par preuves (même score, date à ± 1 jour), 0 ambiguë, 73 non résolues listées (se résolvent quand les preuves s'accumulent) |
| A3 bis | Matchendirect : garder la date et l'adversaire de chaque match de saison (aujourd'hui seuls les buts), année déduite de la saison sans ambiguïté | dates et adversaires identiques à la page sur les pages réelles de `tests/fixtures/pages_equipes/` | **FAIT le 24/09** : date, adversaire et lien gardés (vérifié sur la page réelle de The New Saints) ; cache des saisons vidé pour relecture avec dates |
| A4 | Contrôles qualité : scores football-data contre scores matchendirect sur les matchs communs ; doublons ; dates | ≥ 98 % d'accord ; désaccords listés, jamais corrigés à la main | **FAIT le 24/09** (`controle_football_data.py`, workflow `journal.yml`, page Système) : 717 matchs communs, 716 accords (99,9 %), 1 désaccord listé (FC Köln - Hoffenheim, 29/08), 0 doublon, 0 date invalide ou future. Taux optimiste par construction (voir `limite` dans le rapport) |
| A4 bis | Assemblage par équipe : Football-Data en base, jours manquants complétés par Matchendirect après vérification des dates exactes (même adversaire à ± 1 jour = même match), source indiquée sur chaque match ; championnats non couverts : Matchendirect seul | aucun match en double sur un échantillon réel (dont MLS/Brésil pour le décalage de date) ; aucun match Football-Data remplacé ; un match Matchendirect provisoire est remplacé par Football-Data dès sa publication (assemblage reconstruit à chaque run, jamais cumulé) | **FAIT le 24/09** (`assemblage_equipes.py`, workflow `journal.yml`) : 12 tests, dont remplacement du provisoire par Football-Data. **À vérifier après le run de ce soir** : matchs provisoires apparus avec leurs dates, aucun doublon |
| A5 | Contrat de transmission au moteur : fichiers et champs documentés, versionnés | README + tests | **FAIT le 24/09** : `contrat_moteur.py` (seul point de lecture du moteur, vérification à chaque run, retour à la version précédente si non conforme), `docs/CONTRAT_MOTEUR.md`, 16 tests. **Chantier A terminé** |
