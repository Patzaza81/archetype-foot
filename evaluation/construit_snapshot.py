...
def construit(chemin_entrees, chemin_precalcul, origine, maintenant):
    ...
    matchs = []
    for entree in entrees["matchs"]:
        s = signaux.get(entree["id"])
        if s is None:
            continue
        publie = s[bm.CLE_BLOC]
        ...
        matchs.append({
            "id": entree["id"], "date": s["date"], "heure": s["heure"], "domicile": s["domicile"], "exterieur": s["exterieur"],
            "competition": " ".join(s["competition"].split()), "url_match": s.get("url_match"),
            ...
            "entree_moteur": entree, "inventaire": inventaire, "choix_publies": choix,
            "rejets": publie.get("rejets", []),
        })
