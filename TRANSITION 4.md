...
## 42. Session du 13/09/2026 (suite) — validation.py, calibration.py, fichiers de configuration

**Livré** :
- `archetype_model/learning/validation.py` (nouveau) : `decoupe_apprentissage_validation()` (découpage CHRONOLOGIQUE, 70/30, jamais aléatoire -- un découpage aléatoire mélangerait des périodes entre les deux zones) et `valide_hors_echantillon()`. Vérifié sur un vrai cas de sur-ajustement construit à la main : une modification qui améliore le ROI de -0,464 à -0,007 sur la zone ancienne mais le dégrade de -0,5 à -0,75 sur la zone récente -- `ameliore_les_deux_zones` renvoie bien `False`, exactement le garde-fou que le bureau d'étude jugeait le plus important (rapport du 12/09/2026, §12).
- `archetype_model/learning/calibration.py` (nouveau) : orchestrateur -- `evaluer_proposition()` enchaîne `garde_fous.autoriser_promotion()` PUIS `validation.valide_hors_echantillon()` (jamais l'inverse, pour ne pas lancer une validation coûteuse sur une proposition déjà hors limites). `promouvoir()`/`rejeter()` sont les SEULES fonctions du projet autorisées à écrire dans `config/adaptive_parameters.json`/`config/adaptive_state.json`, toujours journalisées via `journal.py`. Testé sur de vrais fichiers temporaires (pas des mocks) : promotion réelle de `COTE_MIN` sur disque, rejet réellement journalisé sans toucher la configuration, erreurs explicites sur fichier/paramètre absent.
- `config/adaptive_parameters.json` (nouveau) : les 8 paramètres calibrables avec leurs valeurs actuelles = valeurs d'origine (jamais encore calibrées).
- `config/adaptive_state.json` (nouveau) : état vide initial (`version_active: 0`).
- `audit_permanent.py` : 6 tests sur `validation.py` + 9 tests sur `calibration.py`.

**Vérifié réellement** : 456 (précédent, validation.py) + 8 (calibration.py, le compte exact est 450+6+8=464) = **464 vérités, 0 échec**, exit code 0. Rejeu 68/48 confirmé inchangé. Contrôle anti-fantôme fait avant livraison.

**AVERTISSEMENT IMPORTANT, à ne surtout pas perdre de vue** : `config/adaptive_parameters.json` existe maintenant, et `calibration.py` peut calculer des propositions et même les "promouvoir" (écrire dedans) -- mais **`convergence.py` ne lit ce fichier NULLE PART**. Il continue de lire ses 8 seuils comme des constantes Python codées en dur. Tant que `convergence.py` n'est pas modifié pour lire `config/adaptive_parameters.json` au lieu de ses constantes, RIEN de ce qui a été construit depuis la session 34 (archive, résultats, matrice, garde-fous, contrefactuel, validation, calibration) n'a le moindre effet sur la sélection réelle -- c'est un système d'observation et de décision complet, mais encore déconnecté de la production.

**Décision requise avant de continuer** : brancher `calibration.py` sur `convergence.py` demande de modifier, pour la toute première fois du projet, un des trois fichiers protégés (`selector.py`/`convergence.py`/`deduplication.py`). Le changement resterait strictement dans le périmètre déjà autorisé (charger la VALEUR depuis la configuration externe, jamais toucher à la logique de comparaison elle-même -- cahier des charges v2 §2.4), mais touche un fichier jusqu'ici jamais modifié -- ne sera fait qu'après feu vert explicite de Patrick, pas glissé dans un lot de travail plus large.

**Reste, hors périmètre pour l'instant** : `tickets/builder.py`/`tickets/cycle.py` (bloqués -- méthode de corrélation non définie), `learning/team_reference.py` (différé Phase 3), et l'orchestration d'un cycle complet dans `pipeline.yml` (quel script appelle `calibration.py` chaque nuit, avec quelles observations -- pas encore écrit).

## 43. Session du 13/09/2026 (suite) — branchement de convergence.py/robustness.py sur la configuration externe

**Feu vert explicite de Patrick reçu pour cette session.** Première modification touchant, même indirectement, le comportement de `signals/convergence.py` et `poisson/robustness.py` depuis le tout début du projet.

**Livré** :
- `archetype_model/config_loader.py` (nouveau) : seul point de lecture entre le moteur de décision et `config/adaptive_parameters.json`. `valeur_parametre(nom)` retourne la valeur active, avec repli automatique sur les valeurs d'origine documentées (`VALEURS_ORIGINE`) si le fichier est absent, illisible, la clé manquante, ou la valeur non numérique -- jamais un crash, jamais `None` propagé dans le moteur. Avertissement sur stderr uniquement pour un VRAI problème (fichier présent mais mal formé) -- jamais pour l'absence normale du fichier (ex. avant le tout premier déploiement).
- `archetype_model/signals/convergence.py` : les 7 constantes (`COTE_MIN`, `COTE_MAX`, `EDV_MIN_P_*`) proviennent désormais de `config_loader.valeur_parametre()`. **Diff vérifié ligne par ligne contre la version réelle du dépôt** : seules les lignes d'assignation changent, la logique de comparaison plus bas (`if not COTE_MIN <= cote <= COTE_MAX`, etc.) n'a pas bougé d'un caractère.
- `archetype_model/poisson/robustness.py` : même principe pour `ROBUSTNESS_STD_THRESHOLD`, même vérification par diff.
- `audit_permanent.py` : 10 nouveaux tests sur `config_loader.py` (tous les cas de repli vérifiés réellement sur disque : fichier absent, JSON mal formé, clé manquante, valeur non numérique, paramètre inconnu, et la preuve qu'une vraie valeur différente est bien lue) + 3 tests d'intégration prouvant que `convergence.COTE_MIN`/`robustness.ROBUSTNESS_STD_THRESHOLD` proviennent réellement de `config_loader` et non d'une constante restée codée en dur par erreur.

**Vérification la plus critique de cette session** : rejeu 68/48 relancé sur le dépôt modifié -- **identique au bit près**. Confirmé aussi que les valeurs actuellement actives (`COTE_MIN=1.26`, `COTE_MAX=1.74`, `ROBUSTNESS_STD_THRESHOLD=0.08`, etc.) restent rigoureusement identiques aux valeurs d'origine, puisque `config/adaptive_parameters.json` n'a encore jamais été calibré -- **le comportement réel du moteur est, à ce jour, strictement inchangé par ce branchement**. Ce qui change : la porte est maintenant ouverte pour que `calibration.py` produise un effet réel, la prochaine fois qu'il promeut une valeur.

**Vérifié réellement** : 464 (précédent) + 13 = **474 vérités, 0 échec**, exit code 0. Contrôle anti-fantôme fait par diff de contenu contre le vrai dépôt GitHub (pas par date de fichier, qui n'aurait rien prouvé sur une extraction fraîche) -- exactement 4 fichiers modifiés/ajoutés, rien d'autre.
...
## 51. Session du 15/09/2026 — clôture de fenêtre : vérification du run réel post-déploiement, transition vers une nouvelle fenêtre

**Contexte** : après la livraison de la session 50 (suppression de l'ancien moteur + page système), Patrick a signalé une erreur 404 sur `etat_systeme.json` en ouvrant `systeme.html` -- normal, le pipeline n'avait pas encore tourné depuis l'ajout de cette fonctionnalité. Il a lancé un run manuel (`workflow_dispatch`, `panier_id` vide) pour vérifier.

### VÉRIFIÉ (contre le vrai dépôt, après le run)

- `etat_systeme.json` existe, `genere_le` daté du 15/09/2026 -- généré avec succès par `construit_etat_systeme.py`.
- **Le pipeline tourne en production depuis au moins 2 nuits** (cycles de calibration du 14/09 ET du 15/09 visibles dans `etat_systeme.json`) -- confirme que le déploiement complet des sessions précédentes (archivage, résultats, calibration, tickets, page système) fonctionne réellement de bout en bout en conditions réelles, pas seulement en test.
- `calibre_archetype_model.py` fonctionne correctement en production : sur ces 2 cycles, tous les paramètres sont REJETÉS avec un motif honnête (échantillon insuffisant, 30 observations réelles contre 50-100 requis selon le paramètre) -- comportement exactement attendu à ce stade précoce, aucune promotion prématurée.
- `observe_tickets_archetype_model.py` fonctionne : 1 ticket fictif en attente (`nb_tickets_observation_pending: 1`), aucun encore résolu.
- Bilan comportemental réel à ce jour : 30 observations résolues, ROI global -36,6 % -- un échantillon encore bien trop petit pour tirer une conclusion, à suivre dans la durée, pas un signal d'alerte à ce stade.
- Audit complet réexécuté sur cet état réel : **511 vérités, 0 échec**, rejeu 68/48 confirmé inchangé.
- Aucune trace d'erreur liée aux 3 fichiers supprimés (`dispatch_pipeline.py`, `verification_resultats.py`, `GABARIT_COTES_MANUELLES.json`) dans l'état résultant du dépôt.

### À VÉRIFIER (non confirmé directement, à faire dans la prochaine fenêtre si besoin)

- Le rendu visuel réel de `systeme.html` sur le téléphone de Patrick n'a pas été confirmé après régénération du fichier (seul le contenu JSON et la logique de rendu ont été vérifiés séparément, via jsdom, avant cette session).
- Les logs détaillés de chacune des étapes `continue-on-error` du run (`verifie_resultats_archetype_model.py`, `genere_tickets_reels_archetype_model.py`, `calcule_matrice_archetype_model.py`, `notifie_constat_majeur.py`) n'ont pas été inspectés un par un dans l'interface GitHub Actions -- seule la cohérence du résultat final (fichiers produits, contenu) a été vérifiée.

### DÉCIDÉ

- Transition vers une nouvelle fenêtre de conversation à ce point -- un prompt de transition dédié a été remis à Patrick, combinant une matrice universelle de reprise de projet (fournie par Patrick) avec le contexte spécifique d'Archetype Foot.

### Prochaine étape

Reprendre depuis la nouvelle fenêtre avec le prompt de transition fourni. Chantiers connus encore ouverts, par ordre de priorité suggéré :
1. `garde_fous.verifier_rollback()` toujours jamais appelé par `calibre_archetype_model.py` -- aucun rollback automatique ne se déclenche si un paramètre promu se révèle mauvais après coup.
2. Suivre l'accumulation des observations réelles jusqu'aux seuils (50-100 selon paramètre pour une vraie calibration, 100 observations conjointes par paire de signatures pour un vrai ticket) -- rien à coder, juste laisser le temps s'écouler et surveiller les Issues GitHub de constat majeur.
3. Tickets fictifs (mode observation) : premier ticket résolu attendu prochainement, à surveiller via `systeme.html`.
4. Bureau d'étude tickets réels : la Règle C (100 observations conjointes) rend `tickets/cycle.py` inactif pendant probablement plusieurs mois -- comportement voulu, pas un bug.
